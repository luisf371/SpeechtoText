...
## 2026-06-17 (Linux clipboard handoff)

- Linux text insertion now uses clipboard handoff instead of synthetic paste: `src/platform_support.py::use_clipboard_handoff()` (True on Linux) is the single policy switch. Wayland/KDE blocks synthetic Ctrl+V and direct typing into other windows, so `TextInserter.insert_text()` copies the transcript and returns without a keystroke; the user pastes with Ctrl+V.
- Call the platform policy module-qualified (`platform_support.use_clipboard_handoff()`), never via a bare `from ... import use_clipboard_handoff`, so a single `monkeypatch.setattr("src.platform_support.use_clipboard_handoff", ...)` reaches every caller. `dependency_stubs` in `tests/test_push_to_talk.py` patches it to False so streaming/REST app tests keep behaving as on Windows (the test host is Linux).
- Streaming is gated centrally: `PushToTalkConfig.is_parakeet_streaming_active()` returns False whenever handoff is active, so Linux always takes the complete (non-streaming) REST path and refinement applies again. Audit raw `parakeet_streaming_enabled` reads (init validation, `_start_rest_auto_stop_timer`, GUI validator) and switch them to `is_parakeet_streaming_active()` so they agree with the gate.
- Clipboard copy is Wayland-aware: `TextInserter._clipboard_copy()` prefers `wl-copy` (wl-clipboard) when `WAYLAND_DISPLAY` is set and the binary exists, else falls back to `pyperclip` (xclip/xsel only reach the XWayland clipboard). The "transcript ready" chime is `src/assets/audio/clipboard_ready.wav` (stereo/16-bit/44.1 kHz to match the other beeps) and is played at the app layer in `AudioProcessingService.process_audio()` (gated on `enable_audio_feedback`), not inside `TextInserter`, mirroring how start/stop beeps live in the orchestrator.
- Pre-existing test-isolation trap: `tests/test_text_inserter.py` can fail in the full suite with `'types.SimpleNamespace' object has no attribute 'Controller'` because another module's pynput stub (e.g. hotkey tests) wins in `sys.modules` and lacks `Controller`. Fix is an autouse fixture that sets `text_inserter.keyboard.Controller = MagicMock` before each test rather than relying on the module-level stub.

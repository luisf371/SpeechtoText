...
- Streaming insert should not synthesize `Space`/`Ctrl+V` while the push-to-talk hotkey is physically held; wait briefly for `HotkeyService.is_push_hotkey_active()` to clear so late finalized chunks paste after release instead of being affected by the held hotkey state.

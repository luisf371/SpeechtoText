...
_HOTKEY_SUPPRESSION_LOCK = threading.Lock()
_HOTKEY_SUPPRESSION_COUNT = 0
...
def suppress_hotkey_events():
    """Temporarily ignore global hotkey events generated by this process."""
    global _HOTKEY_SUPPRESSION_COUNT, _HOTKEY_SUPPRESSION_UNTIL

    with _HOTKEY_SUPPRESSION_LOCK:
        _HOTKEY_SUPPRESSION_COUNT += 1
    try:
        yield
    finally:
        with _HOTKEY_SUPPRESSION_LOCK:
            _HOTKEY_SUPPRESSION_COUNT = max(0, _HOTKEY_SUPPRESSION_COUNT - 1)
            _HOTKEY_SUPPRESSION_UNTIL = max(
                _HOTKEY_SUPPRESSION_UNTIL,
                time.monotonic() + _HOTKEY_SUPPRESSION_TAIL_SECONDS,
            )


def hotkey_events_suppressed() -> bool:
    with _HOTKEY_SUPPRESSION_LOCK:
        return (
            _HOTKEY_SUPPRESSION_COUNT > 0
            or time.monotonic() < _HOTKEY_SUPPRESSION_UNTIL
        )


def _reset_hotkey_event_suppression_for_tests() -> None:
    global _HOTKEY_SUPPRESSION_COUNT, _HOTKEY_SUPPRESSION_UNTIL
    with _HOTKEY_SUPPRESSION_LOCK:
        _HOTKEY_SUPPRESSION_COUNT = 0
        _HOTKEY_SUPPRESSION_UNTIL = 0.0
...
class HotkeyService:
    ...
    @staticmethod
    def _debug_hotkey_events_enabled() -> bool:
        return os.environ.get("PUSH_TO_TALK_DEBUG_HOTKEYS", "").lower() in {
            "1",
            "true",
            "yes",
        }
    ...
    def _on_key_press(self, key):
        ...
        if self._debug_hotkey_events_enabled():
            logger.debug(
                "Hotkey press: raw={!r}, normalized={!r}, required_push={}, "
                "required_toggle={}, active_before={}",
                key,
                key_name,
                self.hotkey_keys,
    ...
    def _on_key_release(self, key):
        ...
        if self._debug_hotkey_events_enabled():
            logger.debug(
                "Hotkey release: raw={!r}, normalized={!r}, active_before={}",
                key,
                key_name,
                self.current_keys,
            )
